...
| Boot sequence VPU EEPROM 101014| Major | Severe | VPU specific EEPROM content can not be read. [reboot](#reboot--power-cycle) if persistent [factory  reset](#factory-reset)|
| Boot sequence invalid configuration 101018| Major | Unrecoverable | Bring-up failed and unrecoverable. Manual reconfiguration required. [reboot](#reboot--power-cycle) if persistent [factory  reset](#factory-reset)|
...
| Boot sequence head 101008 | Major | Severe  | The extrinsic calibration saved on the device (`saveInit`) does not match the connected hard at this specific port. [Factory  reset](#factory-reset) to remove all the persistently saved settings or debug the configuration manually for each port. |
...
| VPU 103012 | Moderate / Major | Warning / Severe Error | If only sporadic the system will recover by itself. If persistent or repeated, check the [watchdog errors](#watchdog-errors--timeouts) |
...
| head 102002|  Major |  Severe | If detected the system will enter an error state and block the image acquisition - to reset, [reboot](#reboot--power-cycle). <br>If persistent, [flash the firmware anew](#flash-firmware-anew) to update all respective micro controller versions|
| head 102003| Major | Warning / Severe | The respective head will be blocked from acquiring images until no overvoltage is present--fix the power supply and cables.|
| head 102004| Major | Warning / Severe | The respective head will be blocked from acquiring images until no undervoltage is present--fix power supply and cable hardware|
| head 102005| Major | Unrecoverable | The respective head will be blocked from acquiring images until the system is [rebooted](#reboot--power-cycle)|
...
| di 102017| Moderate| Warning  | If motion compensation is activated but can not be performed due to ego data unavailability or outdated timestamps. [Check your mounting setup](#check-mounting-setup). |
...
| ODS 105007 |  Moderate | Warning | The app performs ego-motion estimations. If no ego-motion is available the application performance may be reduced proportionally to time since ego-motion estimation was lost. <br> No internal escalation is performed if active. <br> [Check your mounting setup](#check-mounting-setup).|
...
| ODS 105012 | Moderate | Warning | The app is configured with development features active, for example the overwriting of velocity information status. This is not suggested for deployment as it may lead to unexpected behavior.|
...
Different camera head imagers, for example 2D RGB and 3D ToF, or 3D ToF imagers of different resolutions, need to be connected to separate deserializers.
...
When the data is not retrieved fast enough or the queue is not popped often enough this error is activated:
...
**Solution strategy:**
+ Solutions are being developer where CPU intensive operations are offloaded to the GPU. This will free up CPU resources for OEM applications - will be available with final FW version 1.0.x
