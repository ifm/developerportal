...
Motion Camera Calibration is a camera calibration software provided by ifm based on a ifm specific checkerboard that estimates the rotational parameters of the camera head with respect to the user coordinate system.

This application is available free of charge included in the embedded O3R firmware (`version>=1.1.30`) and the GUI ifm Vision Assistant (`version >=2.7.6`). To use this application, users must install the ifmVisionAssistant tool, which can be downloaded [here](https://www.ifm.com/us/en/product/OVP800?tab=documents). To get started with the ifmVisionAssistant, please refer to this [documentation](index_iVA.md).
...
10.  Trigger `startMotion` command and move the robot backward in a straight line.
   - Once you click the `startMotion` the internal algorithm starts to grab images ( Don't worry about the commands...! The command will automatically return to `nop`)
...
13. Move the robot orthogonal to the backward motion line and close to the target (ﬁll at least 50% of the image) such that the tool detects all corners of the checkerboard target.
   - The orthogonal offset should be a minimum of 40 cm.

![capture_fram_1.gif](_resources/capture_frame_1.gif)

14. Input **0** in `/configuration/takeStaticBufferIndex` and trigger `configuration/command/*takeStaic` to add this image to the calibration data file. 
...
16. Input **1** in `/configuration/takeStaticBufferIndex` and trigger `configuration/command/*takeStaic` again to add another static image.

17. Now trigger `calibrate` command to calibrate the cameras. If the calibration is successful then you can see the extrinsic parameters as the JSON string.

18. If the calibration is not successful, then the reason for the calibration failure is displayed as a notification `set-RPC failed` and the reason for the calibration failure. The following image shows that the calibration failed because the backward motion of the vehicle is not in a straight line as specified.

![Calibration Failure](_resources/calibration_failure.png)

19. If the calibration is successful, then the results will be displayed in the results will be displayed as the JSON object below the display window.
