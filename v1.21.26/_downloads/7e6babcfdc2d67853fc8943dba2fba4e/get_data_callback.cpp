...
void Callback(ifm3d::Frame::Ptr frame) {
  ...
  std::cout
      << "Sample data from the frame (taken at pixel [50, 50] for each image): "
      << std::endl;
  ...
  std::cout << "Conf: " << std::to_string(conf.at<uint8_t>(50, 50))
            << std::endl;
...
int main() {
  ...
  fg->Start({ifm3d::buffer_id::CONFIDENCE_IMAGE,
             ...
             ifm3d::buffer_id::RADIAL_DISTANCE_NOISE, ifm3d::buffer_id::XYZ});
